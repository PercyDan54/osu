...
namespace osu.Game.Screens.Mvis.SideBar.PluginsPage
{
    public class PluginPiece : CompositeDrawable, IHasTooltip
    {
        ...
        private void load(MvisPluginManager manager)
        {
            ...
            {
                indicator.TooltipText = "Can't disable this plugin";
            ...
            if (Plugin.Flags.Contains(MvisPlugin.PluginFlags.CanUnload))
            {
                unloadButton.Action = () =>
                {
                    dialog.Push(new ConfirmDialog($"Are you sure you want to unload {Plugin.Name}?", () => manager.UnLoadPlugin(Plugin))
                    {
                        BodyText = "This plugin will not be available in this sessions!"
                    });
                };
                unloadButton.TooltipText = "Unload this plugin";
                unloadIcon.Icon = FontAwesome.Solid.TrashAlt;
            }
            else
            {
                unloadButton.TooltipText = "Can't unload this plugin";
